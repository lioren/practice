		...
		class RTree
{
		...
		protected:
				...
				struct Rect
				{
						DATATYPE m_min[NUMDIMS];                      ///< Min dimensions of bounding box 
					 	DATATYPE m_max[NUMDIMS];                      ///< Max dimensions of bounding box 
				};
				...
				struct Branch
				{
						friend bool operator< (Branch n1,Branch n2)
						{
								if(n1.m_rect.m_min[0]!=n2.m_rect.m_min[0])
									return n1.m_rect.m_min[0] > n2.m_rect.m_min[0];
									return n1.m_rect.m_min[1] > n2.m_rect.m_min[0];
						}
						Rect m_rect;                                  ///< Bounds
				...
				void Show2(Node* a_node);
				void Find2(Node* a_node);
				bool IS_DOMINATED(DATATYPE x,DATATYPE y);
...
void RTREE_QUAL::Show()
{
	   Show2(m_root); 
		...
		RTREE_TEMPLATE
void RTREE_QUAL::Show2(Node* a_node)
{	    
		if(a_node->IsInternalNode())  // not a leaf node
		{		
				for(int index = 0; index < a_node->m_count; ++index)
				{
						cout << "minx:" << a_node->m_branch[index].m_rect.m_min[0] << "   ";
						cout << "miny:" << a_node->m_branch[index].m_rect.m_min[1] << "   ";
						cout << "maxx:" << a_node->m_branch[index].m_rect.m_max[0] << "   ";
						cout << "maxy:" << a_node->m_branch[index].m_rect.m_max[1] << endl;

						Show2(a_node->m_branch[index].m_child);
				}
		}
		else // A leaf node
		{
				for(int index = 0; index < a_node->m_count; ++index)
				{
						cout << "real minx:" << a_node->m_branch[index].m_rect.m_min[0] << "   ";
						cout << "real miny:" << a_node->m_branch[index].m_rect.m_min[1] << "   ";
						cout << "real maxx:" << a_node->m_branch[index].m_rect.m_max[0] << "   ";
						cout << "real maxy:" << a_node->m_branch[index].m_rect.m_max[1] << endl;
...
void RTREE_QUAL::Find()
{
	   Find2(m_root); 
		...
		RTREE_TEMPLATE
void RTREE_QUAL::Find2(Node* a_node)
{
//		priority_queue<Point> sky;
//		priority_queue<Branch> node;
		if(a_node->IsInternalNode())
		{
				for(int index=0;index < a_node->m_count;++index){
						branch.push(a_node->m_branch[index]);
				}
		}
		while(!branch.empty()){
				Branch a = branch.top();
				branch.pop();
		//		if((a.m_child->m_branch[1].m_child)!=NULL)
		//				printf("Not NULL\n");
		//		else printf("NULL\n");
				if(IS_DOMINATED(a.m_rect.m_min[0],a.m_rect.m_min[1])){						
						if((a.m_child!=NULL)){
								for(int index=0;index< a.m_child->m_count;++index){
										if(IS_DOMINATED(a.m_child->m_branch[index].m_rect.m_min[0],a.m_child->m_branch[index].m_rect.m_min[1])){
												branch.push(a.m_child->m_branch[index]);
										}
								}
						}
						else{
								//cout << "sky point++" << endl;
								Point skp;
								skp.x= a.m_rect.m_min[0];
								skp.y= a.m_rect.m_min[1];
							//	cout << skp.x << "  " << skp.y << endl;
								sky.push_back(skp);
						}
				}
		...
		typename vector<Point>::iterator it;
				cout << "Skyline Point" << endl;
		for(it=sky.begin();it!=sky.end();++it)
		
				cout << it->x << "   " <<  it->y << endl;


//		Branch a = node.top();
//		cout << a.m_child->m_branch[1].m_rect.m_min[0] << " ";
//		cout << a.m_child->m_branch[1].m_rect.m_min[1] << " ";
	   
}
		RTREE_TEMPLATE
bool RTREE_QUAL::IS_DOMINATED(DATATYPE x, DATATYPE y)
{
		if(sky.empty())
				return  true;
		typename vector<Point>::iterator it; 
