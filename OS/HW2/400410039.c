...
int main(void)
{
		...
		while (fgets(buf, MAXLINE, stdin) != NULL) {
				...
				if(pipeline_count!=0){
						...
						{
								int fd_in;
								if(C==0)fd_in=STDIN_FILENO;
								else fd_in=pipes_fd[C-1][0];
								int fd_out;
								if(C==cmd_count-1)fd_out=STDOUT_FILENO;
								else fd_out=pipes_fd[C][1];
								pid_t proc = fork();
								if (proc < 0)
								{
								...
								else if (proc == 0)
								{
										...
										if(strcmp(arg[C+1][0],"|")==0){
												if(execvp(arg[C+1][1],&arg[C+1][1])==-1){
														fprintf(stderr,"Error");
														//		exit(EXIT_FAILURE);
												}
										}
										else{
												if (execvp(arg[C+1][0],arg[C+1]) == -1)
												{
