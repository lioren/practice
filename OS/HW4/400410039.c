...
int main () {
		...
		int i,j,k;
		srand(time(NULL));
		printf("Please insert number of n:");
		scanf("%d",&len);
		pthread_t t[len*2];
		...
		for(k=0;k<len;k++){
				...
				*ptr = k ;
				pthread_create(&t[k], NULL, row_do, ptr); 
		}
		for(i=0;i<len;i++){
		...
		for(k=0;k<len;k++){
				int *ptr = malloc(sizeof(int));
				*ptr = k ;
				pthread_create(&t[k], NULL, column_do, ptr);
		}
		for(i=0;i<len;i++){
				pthread_join((pthread_t) t[i], (void **) NULL); 
		}
